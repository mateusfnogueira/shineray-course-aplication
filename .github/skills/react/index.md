---
name: react
description: Convenções React e Next.js para admin-web e client-web (App Router, RSC, Tailwind). Para apps/marketing usar a skill `marketing`.
type: skill
---

# React / Next.js — MareApp

> Esta skill cobre `admin-web` e `client-web`.
> Para `apps/marketing` (landing page institucional), usar a skill **`marketing`** — tem convenções próprias de SSG, SEO e animações.
...
- `admin-web` e `client-web`: mobile-first, contraste alto, `min-h-[48px]` em elementos tocáveis
- `apps/marketing`: **mobile-first**, SSG puro, sem Auth.js — ver skill `marketing`
- Decisão completa: ADR-027
